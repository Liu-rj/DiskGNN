...
x_labels = [all_configs[0][0], all_configs[3][0], all_configs[6][0], all_configs[9][0]]
    ...
    for j in range(n):
        ...
        plot_label = [
            ...
            for data in all_new_normalized_statistics[
                (line_num - 1) * 6 + id * group : (line_num - 1) * 6 + id * group + 3
            ]
        ]
...
plt.savefig(f"{SAVE_PTH }/vary_mem_camera_ready.pdf", bbox_inches="tight", dpi=300)
## print save
print(f"Save to {SAVE_PTH }/vary_mem_camera_ready.pdf")
