...
file_path = "../offgs_data/breakdown_method1.csv"
...
hatches = ["x", "//", "\\\\", "--", ""]
hatches = hatches[::-1]
labels = ["Raw", "Raw+P", "Raw+P+G", "Raw+P+G+C", "Raw+P+G+C+D"]
...
for it, ydata in enumerate([all_normalized_runtime, all_speedup]):
    ...
    if it == 0:
        ...
        tx_yticks = np.arange(-10, 51, 10)
    else:
        yticks = np.arange(0, 51, 10)
        tx_yticks = np.arange(0, 51, 10)
    ...
    file_name = f"{SAVE_PTH}/breakdown_method1_{file_label}.pdf"
