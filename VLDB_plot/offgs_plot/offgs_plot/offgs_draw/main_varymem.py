...
for line_num in line_nums:
    ...
    for i in range(2):
        ...
        axes[i].set_xticklabels(['10\%','30\%','50\%'], fontsize=8)
        ...
        for j in range(n):
            ...
            container=axes[i].bar(
                exit_idx_x + j * width,
                [data[j] if data[j]<val_limit else val_limit for data in all_new_normalized_statistics[(line_num-1)*6+i*group:(line_num-1)*6+i*group+3]],
                width=width * 0.8,
                color="white",
                edgecolor='k',
                hatch=hatches[j ],
