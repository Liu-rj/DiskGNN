...
for i, (label, dataset) in enumerate(zip(datasets, cache_data)):
    ...
    # Plot bars
    container=ax.bar(exit_idx_x + i * width, dataset, width=width*0.8, 
           
           color='w',edgecolor='black',hatch=hatches[i],linewidth=1,label=labels[i],zorder=10)
    ax.bar_label(container, dataset, fontsize=11, zorder=200, fontweight="bold")
