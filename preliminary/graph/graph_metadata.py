...
a = g.out_degrees().to("cuda")
a, idx = torch.sort(a, dim=0, descending=True)
b = (torch.cumsum(a, dim=0) / g.num_edges()).cpu().detach().numpy()
font_size = 10
plt.plot(np.arange(b.shape[0]) / g.num_nodes(), b)
plt.xlabel(
    "proportion of nodes in descending order of out-degrees",
    fontsize=font_size,
    fontweight="bold",
)
plt.ylabel(
    "proportion of accumulated out-degrees", fontsize=font_size, fontweight="bold"
)
plt.title(name + "-accumulated-out-degrees", fontsize=font_size, fontweight="bold")
plt.grid(linestyle="-.")
plt.savefig(name + "-outdegree-distribution.pdf", bbox_inches="tight")
plt.clf()
