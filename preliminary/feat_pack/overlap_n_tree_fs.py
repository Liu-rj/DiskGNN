...
class Jaccard_TreeBuilder:
    ...
    def jaccard_similarity(t1, t2):
        if len(t1) == 0 or len(t2) == 0:
            return 0,0
        # Assuming t1 and t2 are lists, convert them to tensors
        t1_tensor = torch.tensor(t1) if isinstance(t1, list) else t1
        t2_tensor = torch.tensor(t2) if isinstance(t2, list) else t2

        # Now concatenate the tensors
        combined = torch.cat((t1_tensor, t2_tensor))
...
def run_jaccard(dataset, args):
    ...
    num_batches = (train_nid.numel() + args.batchsize - 1) // args.batchsize
    # num_batches = math.ceil(num_batches / 10)
    table_size = 4 * dataset.num_nodes
    num_entries = min((args.feat_cache_size - table_size) // (4 * features.shape[1]), dataset.num_nodes)
    if num_entries > torch.iinfo(torch.int32).max:
    ...
    ## we can optimize the difference set process
    cold_index_strs=[]
    ## get 2 random num in num_batches
    random_num=np.random.randint(0,num_batches,size=2)
    jaccard_simliarity_list=[]
    
    ## iterate the random num
    for i , random_num_element in enumerate(random_num):
        score_list=[]
        random_input_nodes: torch.Tensor = torch.load(f"{output_dir}/in-nid-{random_num_element}.pt").to("cuda")
        rev_hot_idx = torch.isin(random_input_nodes, cache_indices, assume_unique=True).nonzero(as_tuple=True)[0]
        hot_nodes = random_input_nodes[rev_hot_idx]
        rev_cold_idx = torch.isin(random_input_nodes, hot_nodes, assume_unique=True, invert=True).nonzero(as_tuple=True)[0]
        cold_nodes = random_input_nodes[rev_cold_idx]
        random_cold_list=cold_nodes.tolist()
    
        for i in trange(num_batches):
            if i > 0.05 * num_batches:
                break
            if i==random_num_element:
                continue
            ...
            torch.cuda.synchronize()
            difference_time += time.time() - tic
            cold_nodes_list=cold_nodes.tolist()
            score,_=Jaccard_TreeBuilder.jaccard_similarity(random_cold_list,cold_nodes_list)
            score_list.append(score)
            
        jaccard_simliarity_list.append(score_list)
        
    ## plot two jaccard similarity distribution and put the two curve in one figure
    plt.hist(jaccard_simliarity_list[0], bins=2500, alpha=0.7)
    plt.hist(jaccard_simliarity_list[1], bins=2500, alpha=0.7)
    plt.title("Value Distribution of intersection_jaccard")
    plt.xlabel("Values")
    plt.ylabel("Frequency")
    plt.grid(True)
    ## save the figure
    plt.savefig(f'jaccard_simliarity_distribution_{args.feat_cache_size}.png')
        # cold_index_strs.append(cold_nodes)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="igb-full")
