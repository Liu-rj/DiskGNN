...
def train(
    ...
    subg_dir = f"{args.dir}/{args.dataset}-{args.batchsize}-{args.fanout}-{args.ratio}"
    ...
    print("Label Ratio:", train_num / dataset.num_nodes)
    ...
    for epoch in range(args.num_epoch):
        ...
        for i in trange(size, ncols=100):
            if len(sample_pool) == 0:
                sample_pool = list(range(pool_size))
            random_index = np.random.randint(0, len(sample_pool))
            block_id = sample_pool.pop(random_index)

            tic = time.time()
            blocks = torch.load(f"{subg_dir}/train-{block_id}.pt")
            output_nodes = torch.load(f"{subg_dir}/out-nid-{block_id}.pt")
            input_nodes = torch.load(f"{subg_dir}/in-nid-{block_id}.pt")
            ...
            y = gather_pinned_tensor_rows(
                labels, output_nodes.to(device) - label_offset
            ).long()
            torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log", type=str, default="../logs/train_offline_acc.csv")
