...
def evaluate(
    ...
    for it, (input_nodes, output_nodes, subg) in enumerate(
...
def train(
    ...
    acc_logfile = f"logs/offline_{args.dataset}_{args.fanout}_{args.model}_{args.hidden}_{args.dropout}_{args.ratio}_{args.batchsize}.csv"
    ...
    for epoch in range(args.num_epoch):
        ...
        for it, i in tqdm(enumerate(batch_id), ncols=100):
            ...
            subg = subg.to(device, nonblocking=True)
...
def start(args):
    ...
    cpu_cached_feats, gpu_cached_feats = None, None
    
    cpu_num_entries = args.cpu_cache_size // (4 * dataset.num_features)
    gpu_num_entries = args.gpu_cache_size // (4 * dataset.num_features)

    args.cpu_cache_ratio = cpu_num_entries / dataset.num_nodes
    args.gpu_cache_ratio = gpu_num_entries / dataset.num_nodes
