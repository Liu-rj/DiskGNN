...
def load_meta(feat_load_queue, transfer_queue, aux_dir, batch_id, subg_dir):
    for i in batch_id:
        ...
        feat_load_queue.put((input_nodes, rev_cold_idx, mem_loc, rev_hot_idx))
        transfer_queue.put((input_nodes, rev_cold_idx, mem_loc, rev_hot_idx))


def load_feats(in_queue, out_queue, batch_id, dataset, gpu_cached_feats):
    torch.ops.offgs._CAPI_Init_iouring()
    for i in batch_id:
        input_nodes, rev_cold_idx, mem_loc, rev_hot_idx = in_queue.get()
        rev_cpu_hot_idx = rev_hot_idx[mem_loc >= gpu_cached_feats.shape[0]]
        rev_idx = torch.cat([rev_cold_idx, rev_cpu_hot_idx])
        feats = torch.ops.offgs._CAPI_GatherIOUringDirect(
            dataset.features_path, input_nodes[rev_idx], dataset.num_features
        )
        out_queue.put((feats, rev_idx))
...
def feat_transfer(
    ...
    for i in batch_id:
        input_nodes, rev_cold_idx, mem_loc, rev_hot_idx = in_queue.get()
        disk_feats, rev_idx = disk_feats_queue.get()
        ...
        if num_disk > 0:
            x[rev_idx] = disk_feats.to(device)

        gpu_loc = mem_loc[mem_loc < gpu_cached_feats.shape[0]]
        rev_gpu_hot_idx = rev_hot_idx[mem_loc < gpu_cached_feats.shape[0]]
        x[rev_gpu_hot_idx] = gpu_cached_feats[gpu_loc]
        out_queue.put(x)
