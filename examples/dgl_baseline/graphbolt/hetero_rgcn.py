...
def evaluate(
    ...
    loader_len = (item_set[:][category][0].numel() + 4096 - 1) // 4096

    for data in tqdm(
        data_loader, desc="Inference", total=loader_len, ncols=100
    ):
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for data in tqdm(
            data_loader,
            desc=f"Training~Epoch {epoch + 1:02d}",
            total=loader_len,
            ncols=100,
        ):
