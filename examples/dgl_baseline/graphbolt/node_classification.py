    ...
    def inference(self, graph, features, dataloader, storage_device):
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            for data in tqdm(dataloader):
                ...
                y[data.seeds[0] : data.seeds[-1] + 1] = hidden_x.to(buffer_device)
            if not is_last_layer:
...
@torch.no_grad()
def layerwise_infer(args, graph, features, test_set, all_nodes_set, model, num_classes):
    model.eval()
...
def train(args, graph, features, train_set, valid_set, num_classes, model):
    ...
    loader_len = (train_set[:][0].numel() + args.batch_size - 1) // args.batch_size
...
def main(args):
    ...
    dataset = gb.BuiltinDataset(
        args.dataset, root="/nvme2n1/graphbolt_dataset/datasets"
    ).load()
    ...
    # memory_holder = torch.zeros(1024 * 1024 * 1024 * 80, dtype=torch.int8)
