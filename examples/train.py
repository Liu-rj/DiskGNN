...
def load_feats(queue, aux_path, size, cached_feats, address_table):
    for i in range(size):
        cold_feats, cold_nodes, hot_nodes, rev_hot_idx, rev_cold_idx = torch.ops.offgs._CAPI_LoadFeats_ODirect(
            f"{aux_path}/train-aux-{i}.npy", dataset.num_features, 10240
        )
...
def train(args, dataset: OffgsDataset, address_table, cached_feats, subg_dir, aux_dir):
    ...
    for epoch in range(args.num_epoch):
        ...
        feats_loader = threading.Thread(target=load_feats, args=(feats_queue, aux_dir, size, cached_feats, address_table))
        feats_loader.start()
