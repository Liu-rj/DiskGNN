...
def load_feats(queue, aux_path, size):
    for i in range(size):
        aux_data = torch.load(f"{aux_path}/train-aux-{i}.pt")
        feats, meta = aux_data[0], aux_data[1:]
        queue.put((feats, meta))
...
def train(args, dataset, address_table, cached_feats, subg_dir, aux_dir):
    ...
    for epoch in range(args.num_epoch):
        ...
        graph_loader = threading.Thread(target=load_graph, args=(graph_queue, subg_dir, size))
        graph_loader.start()

        feats_queue = Queue(maxsize=5)
        feats_loader = threading.Thread(target=load_feats, args=(feats_queue, aux_dir, size))
        feats_loader.start()
        ...
        for i in trange(size):
            ...
            tic = time.time()
            blocks = graph_queue.get()
            graph_load_time += time.time() - tic
            tic = time.time()
            cold_feats, meta = feats_queue.get()
            feats_load_time += time.time() - tic
            cold_feats_num += cold_feats.shape[0]

            tic = time.time()
            cold_nodes, hot_nodes, rev_hot_idx, rev_cold_idx = meta
