...
def run(dataset: OffgsDataset, args):
    ...
    print(
        f"#Cached Entries: {num_entries} / {total_num_nodes}",
        f"Ratio: {num_entries / total_num_nodes}",
    ...
    if args.blowup == -1:
        ...
        target_size = dataset.conf["total_num_nodes"] * args.blowup
    ...
    assert disk_cache_num + num_entries < dataset.conf["total_num_nodes"]
    ...
    for segid in trange(num_segments, ncols=100):
        ...
        for node_type in dataset.conf["num_nodes"].keys():
            ...
            if (popularity > 0).sum().item() > 0:
                tqdm.write(
                    f"\nDisk Cache Entries: {cache_num} / {total_num_nodes}\t"
                    f"Ratio: {cache_num / total_num_nodes:.3f}\t"
            ...
            all_disk_cache[node_type] = (disk_key, disk_value)
            ...
            # tic = time.time()
            # reordered_disk_cache = reordered_disk_cache + dataset.conf["node_offsets"][node_type]
            ...
            #     f"{aux_dir}/disk_cache/disk-cache-{segid}-{node_type}.bin", disk_cache_feats
            # )
        ...
        for bid in range(startid, endid):
            for node_type in dataset.conf["num_nodes"].keys():
                ...
                tic = time.time()
                torch.ops.offgs._CAPI_SaveFeats(
                    f"{aux_dir}/feat/train-aux-{bid}-{node_type}.bin", packed_feats
                )
    ...
    print(
        ...
        f"Original Blowup: {ori_io / total_num_nodes:.3f}\t"
        f"Opt Blowup: {total_disk_nodes / total_num_nodes:.3f}\t"
