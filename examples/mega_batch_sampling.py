...
def run(args, dataset, label_offset):
    ...
    tic = time.time()
    for i, (input_nodes, output_nodes, blocks) in tqdm(
        enumerate(train_dataloader), total=len(train_dataloader), ncols=100
    ):
        ...
        rev_idx = torch.ops.offgs._CAPI_IndexSearch(
            subgraph.ndata[dgl.NID].to("cuda"),
            output_nodes.to("cuda"),
        )
        input_nodes = subgraph.ndata[dgl.NID].clone()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--log",
        type=str,
        default="logs/sample_megabatch_decompose.csv",
        help="log file",
    )
